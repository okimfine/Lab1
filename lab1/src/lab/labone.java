  ...
  public static String[] VerCreate(String txt) {
   ...
   public static String queryBridgeWords(String word1, String word2) {
	   ...
	   if (!Arrays.asList(v).contains(word1)
			   && !Arrays.asList(v).contains(word2)) { //待查询单词不在图中
		   System.out.println("No " + "\"" + word1 + "\"" + " and "
			   + "\"" + word2 + "\"" + " in the graph!");
	   } else if (!Arrays.asList(v).contains(word1)) {
		   System.out.println("No " + "\"" + word1 + "\""
			   + " in the graph!");
	   } else if (!Arrays.asList(v).contains(word2)) {
